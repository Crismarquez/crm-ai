...
from strong_sort.deep.reid.torchreid.utils import FeatureExtractor
...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(
        ...
        if self.pt:  # PyTorch
            ...
            if not file_exists(w) and model_url is not None:
                ...
                print(
                    f"No URL associated to the chosen DeepSort weights ({w}). Choose between:"
                )
                show_downloadeable_models()
                exit()
                ...
                if model.binding_is_input(index):
                    if -1 in tuple(model.get_binding_shape(index)):  # dynamic
                        ...
                        context.set_binding_shape(
                            index, tuple(model.get_profile_shape(0, index)[2])
                        )
                    if dtype == np.float16:
                        fp16 = True
    ...
    def preprocess(self, im_crops):
        def _resize(im, size):
            return cv2.resize(
                cv2.cvtColor(im, cv2.COLOR_BGR2RGB).astype(np.float32), size
            )
            ...
            for i in range(0, im_batch.shape[0]):
                ...
                #    y = self.extractor.model(im)[0]
                    ...
                    if self.dynamic and im.shape != self.bindings["images"].shape:
                        i_in, i_out = (
                            self.model.get_binding_index(x)
